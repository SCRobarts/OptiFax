function [Z, AT, AW, W, B] = gnlsefib(T, A, w0, gamma, betas, ...
...
%	T = time array / s
%	A = E(t) field amplitude / sqrt(W)
%	w0 = central angular frequency of pulse? / rad/s
%	gamma = nonlinearity / (W.m)^-1
%	betas = beta coefficients = [beta2, beta3, ...] / [s^2/m, s^3/m ...]
%	loss = absorption etc. / dB/m
%	fr = Raman fraction (0.18 for FS)
%	RT = Raman response function in the time domain?
%	flength = fibre length / m ?
%	nsaves = number of points along z to retrieve progress field
...
% RW = n*fft(ifftshift(RT.'));   % frequency domain Raman
L = fftshift(L); W = fftshift(W); % shift to fft space
% L = ifftshift(L); W = ifftshift(W); % shift to fft space

% === define function to return the RHS of Eq. (3.13)
% i.e gives del_z(AW)
function R = rhs(z, AW)
  AT = fft(AW.*exp(L*z));         % time domain field
  % AT = ifft(AW.*exp(L*z));         % time domain field
  ...
  if (length(RT) == 1) || (abs(fr) < eps) % no Raman case
	...
	% RS = dT*fr*ifft(fft(IT).*RW); % Raman convolution
    % M = fft(AT.*((1-fr).*IT + RS));% response function
  end
...
% [Z, AW] = ode23(@rhs, Z, ifft(A), options); % run integrator
...
for i = 1:length(AW(:,1))
  ...
  % AT(i,:) = ifft(AW(i,:));           % time domain output
  AW(i,:) = fftshift(AW(i,:)).*dT*n;  % scale
  % AW(i,:) = ifftshift(AW(i,:)).*dT*n;  % scale
end
